# InteractUI_2AFC: interact_2AFC.py
...
from task.task_2AFC import Task2AFC

# load the .ui files
Interact_class = uic.loadUiType("ui/interact_2AFC.ui")[0]


class InteractUI_2AFC(QtGui.QMainWindow, Interact_class):
    ...
    def __init__(self, parent=None):
        ...
        # --- button actions --- #
        # shrew feedback
        self.btnGiveRewardLeft.clicked.connect(self.btn_give_left_reward_clicked)
        self.btnGiveRewardRight.clicked.connect(self.btn_give_right_reward_clicked)
    ...
    def set_task(self, task):
        ...
        self.setGeometry(sdGeom.right() + 20, sdGeom.top(), geom.width(),
                         geom.height())
    ...
    # --- shrew feedback --- #
    def btn_give_left_reward_clicked(self):
        ...
        self.training.leftsyringeSerial.write(rewardMicroliters + "\n")
        self.log_and_print("User gave left reward: " + rewardMicroliters)
        self.training.log_plot_and_analyze("user_reward_left:" +
                                           str(int(rewardMicroliters)/1000),
                                           time.time())
        self.training.daq.send_stimcode(STIMCODE_REWARD_GIVEN_LEFT)

    def btn_give_right_reward_clicked(self):
        """Delivers a reward based on button click. The bolus volume is
        defined by the number entered in the self.txtRewardSize text box."""

        rewardMicroliters = str(self.txtRewardSize.text())
        self.training.rightsyringeSerial.write(rewardMicroliters + "\n")
        self.log_and_print("User gave right reward: " + rewardMicroliters)
        self.training.log_plot_and_analyze("user_reward_right:" +
                                           str(int(rewardMicroliters)/1000),
                                           time.time())
        self.training.daq.send_stimcode(STIMCODE_REWARD_GIVEN_RIGHT)
    ...
    def btn_replace_next_clicked(self):
        ...
        if not isinstance(self.task, Task2AFC):
    ...
    def btn_splus_grating_clicked(self):
        ...
        if not isinstance(self.task, Task2AFC):
            # doesn't make sense on headfix task
            return
        ori = random.choice(self.task.sRightOrientations)
        stimCommand = "sqr" + str(ori) + " as sf0.25 tf0 jf3 ja0.25 px0 py0 sx999 sy999"
        self.training.stimDevice.write(stimCommand)
        self.log_and_print("Set screen to RIGHT grating: " + str(ori))
        self.training.daq.send_stimcode(STIMCODE_SRIGHT_SCREEN)
        # sleeping on the UI thread, such hacks *shudder*
        time.sleep(0.5)
    ...
    def btn_sminus_grating_clicked(self):
        ...
        ori = random.choice(self.task.sLeftOrientations)
        stimCommand = "sqr" + str(ori) + " as sf0.25 tf0 jf3 ja0.25 px0 py0 sx999 sy999"
        self.training.stimDevice.write(stimCommand)
        self.log_and_print("Set screen to LEFT grating: " + str(ori))
        self.training.daq.send_stimcode(STIMCODE_SLEFT_SCREEN)
        # sleeping on the UI thread, such hacks *shudder*
        time.sleep(0.5)
        self.training.stimDevice.write("sx0 sy0")   # back to gray screen
        self.training.daq.send_stimcode(STIMCODE_GRAY_SCREEN)
