# TwoAFCOpenEndAnalysis: task2afc_openend_analysis.py
# Author: Matthew McCann (@mkm4884)
# Max Planck Florida Institute for Neuroscience
# Last Modified: 01/09/2017

# Class containing analysis functions for the open-ended 2AFC task developed
# for Doris Tsao's group. Reads in the log and settings files, and parses the
# log file to reconstruct the experiment and display the statistics on the UI.
...
from trial.twoAFCOpenEnd_trial import *
...
class TwoAFCOpenEndAnalysis:
    ...
    def __init__(self, logFile=None, settingsFile=None, notesFile=None):
        ...
        # make first trial
        self.t = TwoAFCOpenEndTrial(analysis=self)
    ...
    def read_settings_file(self, settingsFile):
        ...
        m = re.match("(.*)_(.*)_(\\d+)_settings.txt",
                     settingsFile.split("\\")[-1])
        self.shrewName = m.group(1)
    ...
    def trial_stats(self):
        ...
        # Task error rate
        self.taskLeftErrors = results["MISS_LEFT"]
        self.taskRightErrors = results["MISS_RIGHT"]
        ...
        # Updated for open ended 2AFC, since misses and task failures are
        # considered the same thing (i.e. the reward period and the stim
        # period are the same)
        ...
        for t in self.trials:
            ...
            if t.numSLeft == max(self.sLeftPresentations) and len(self.sLeftPresentations) > 1:
                ...
                # Changed since now MISS_LEFT == TASK_FAIL_LEFT
                elif t.result == Results.MISS_LEFT:
                    self.sLeftPerformances[sLeftOri].numTrials += 1
            else:
                # it's an sRight trial
                ...
                # Changed since now MISS_RIGHT == TASK_FAIL_RIGHT
                elif t.result == Results.MISS_RIGHT:
                    self.sRightPerformances[sRightOri].numTrials += 1

        # duration
        self.trainingDuration = (self.trials[-1].trialStartTime -
                                 self.trials[0].trialStartTime) / 60 / 60
    ...
    def str_overview(self):
        ...
        message = (
            ...
            '% (' + str(self.sLeftCorrect+self.sRightCorrect) + '/' +
             str(self.ValidTrials) + ')' + "\n"
            'Valid Trial Rate: ' + str(round(self.ValidRate, 2)) +
            '% (' + str(self.ValidTrials) + '/' +
             str(self.nTrials) + ')' + "\n" 
            '\nTotal Reward (mL): ' + str(self.totalmL) + "\n")

        if self.hintmL > 0:
            message += "Reward from Hints (mL): " + str(self.hintmL) + \
                       " (" + str(round(100*self.hintmL/self.totalmL)) + \
                       "% of total)\n\n"
        ...
        message += (
            ...
            "Mean Time Between Trials: " +
            str(round(self.meanTimeBetweenTrials, 2)) + "s\n"
            "\n"
        )
        return message
    ...
    def str_accuracy(self):
        ...
        for sRightOrientation in sorted(set(self.sRightOrientations)):
            ...
            oriStr = "movie: " + str(sRightOrientation)
        ...
        for sLeftOrientation in sorted(set(self.sLeftOrientations)):
            ...
            oriStr = "movie: " + str(sLeftOrientation)
            oriStr += " " * (17-len(oriStr))
            oriStr += successRateStr + "% (" + str(numCorrect) + "/" + str(numTrials) + ")"
            oriStr += "\n"
    ...
    def str_task_errors(self):
        ...
        message = (
            '====' + "\n"
            "TASK ERRORS\n" + "\n"
            "Left Task Error Rate: " + str(self.taskLeftErrorRate) + "% (" +
            str(self.taskLeftErrors) + "/" + str(self.nTrials) + ")" + "\n"
            "Right Task Error Rate: " + str(self.taskRightErrorRate) + "% ("
            + str(self.taskRightErrors) + "/" + str(self.nTrials) + ")" + "\n")
        ...
        if self.resultCounts["NO_RESPONSE_LEFT"] > 0:
            message += 'NO_RESPONSE_LEFT ' + \
                       str(self.resultCounts["NO_RESPONSE_LEFT"]) + "\n"
        ...
        if self.resultCounts["NO_RESPONSE_RIGHT"] > 0:
            message += 'NO_RESPONSE_RIGHT ' + \
                       str(self.resultCounts["NO_RESPONSE_RIGHT"]) + "\n"
    ...
    def get_results_str(self):
        ...
        return self.str_overview() + \
               self.str_accuracy() + self.str_task_errors()
