...
#        print str(self.optoMode)

        if self.useOpto:
            self.training.optoController.write(self.setup_cmd)

            #if not hasattr('self','optoMode'):
                #self.optoMode = OptoMode.RANDOM

            #if not hasattr('self', 'optoBlockSize'):
                #self.optoBlockSize = 1
    ...
    def make_trial_set(self):
        ...
        if self.useOpto and self.optoMode == OptoMode.BLOCK:
            print ('making led sequencer')
            self.ledSet = []
            for x in self.powerLevels:
                tmp = [x] * self.optoBlockSize
                self.ledSet.extend(tmp)
            self.ledSequencer = Sequencer(self.ledSet, self.ledSequenceType)
    ...
    def check_state_progression(self):
        ...
        if self.state == States.REWARD:
            ...
            # -- success condition -- #
            if self.lastLickAt >= self.stateStartTime + self.rewardDelay:
                if random.uniform(0, 1) < self.catchChance:
                    ...
                    self.training.dispense_reward(0)
    ...
    def change_state(self, newState):
        ...
        if (newState == States.TIMEOUT):
            ...
            # here we will reassign self.currentTrial.ledPower based on our block scheme
            # check self.trialResult, if it's not a DELAY or NO_RESPONSE, then iterate block counter
            if self.useOpto and self.optoMode == OptoMode.BLOCK:
                self.currentPower = self.ledSequencer.getNextTrial(self.trialResult)
                self.currentTrial.ledPower = self.currentPower
